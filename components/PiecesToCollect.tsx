    ...
    <div className="bg-white rounded-[2rem] p-6 md:p-8 soft-shadow border border-neutral-border hover:border-brand-light transition-all relative flex flex-col group h-full overflow-hidden">
      ...
      <div className="pt-1">
        <button 
          ...
          className={`w-full py-4 rounded-xl text-[12px] font-extrabold uppercase tracking-widest transition-all ${
            !action 
        ...
        <div className="flex bg-white p-1.5 rounded-full border border-neutral-border soft-shadow overflow-x-auto no-scrollbar w-full md:w-auto">
          {(['all', '1era_quema', 'esmaltado', 'a_recogida'] as const).map(s => (
            <button 
              ...
              className={`px-6 lg:px-10 py-3 rounded-full text-[13px] lg:text-[15px] uppercase tracking-widest transition-all whitespace-nowrap ${filterStatus === s ? 'bg-brand text-white font-black soft-shadow' : 'text-neutral-textSec font-black hover:text-brand'}`}
            >
